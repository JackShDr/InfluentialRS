...
class DataProvider():
    ...
    def __init__(self, config, verbose = False):
        ...
        self.data, self.n_user, self.n_item = self._load_data(self.path, first = config.first, threshold = config.infrequent_thredshold) 
    ...
    def _load_data(self, path, first = True, threshold = None):
        ...
        if first == False: # If do not re-generate the dataset
            ...
            if self.verbose == True:
                ...
                print("============== End ==============\n")
            return data_pack["data"], data_pack["user_count"], data_pack["item_count"]
        ...
        if self.dataset == "ml-1m":
            ...
            temp = '[%s]' % ','.join(data_str.splitlines()) # Replace the line split 
            temp = "{'data':" + temp + "}" # Adjust the json string
            temp = json.dumps(ast.literal_eval(temp)) # Replace single quote with double quote
        ...
        if threshold!= None:
            ...
            # The order to remove infrequent item
#            column_order = ["userId"] # Recursively remove may lead to empty datset
#            for col in column_order:
#                counts = data[col].value_counts(dropna=False)
#                data = data[data[col].isin(counts[counts.isin(list(range(threshold,counts.max())))].index)]
...
##                
            ...
            while not stop_remove:
                ...
                for col in column_order:
                    ...
                    idx = data[col].isin(counts[counts.isin(list(range(threshold,counts.max())))].index)
                    data = data[idx]
                    data.reset_index(drop=True, inplace = True)
#                    print("---")
#                    print(sum(counts<threshold))
#                    print(len(data))
...
#                    stop_remove = True
        ...
        pack = {"user_count": user_count, "item_count": item_count, "data": sequences}
    ...
    def _nn1_split_data(self, data, seq_len = 100, min_len = 40):
        ...
        print("Generate dataset for nn1 evaluator...\n")
        seqs = []
        seqs_count = 0
        seq_len += 1
    ...
    def _random_split_data(self, data, seq_len = 100, min_len = 40, step = -1):
        """
        Split the data into training and validation set. Used by PrsNN.  
        """
        # Generate subsequences from user sequences
        print("Generate dataset for PrsNN...\n")
        seqs = []
        seqs_count = 0
        for user_id in range(len(data)): # for every single user sequence
            # Generate training sequences
...
#                print(len(new_seq))
    ...
    def get_refer_data(self, load = False, save = False, file_class = "prs", train_ratio = 0.9, \
        ...
        if load == False: # If do not load existing dataset
            ...
            if file_class == "prs":
                all_seqs = self._random_split_data(data = self.data, seq_len = seq_len, min_len = min_len, step = step) 
            elif file_class == "session":
                all_seqs = self._session_split_data(data = self.data, seq_len = seq_len, min_len = min_len,step = step)
            elif file_class == "nn1":
                all_seqs = self._nn1_split_data(data = self.data, seq_len = seq_len, min_len = min_len)
            ...
            if save == True: # Saving data
                np.save(self.path+"%s_train_seq.npy"%file_class, train_seqs)
                np.save(self.path+"%s_valid_seq.npy"%file_class, valid_seqs)
                np.save(self.path+"%s_all_seq.npy"%file_class, all_seqs)
                
            if file_class == "prs": # Save the targets that used in the training dataset
            ...
            print("Load dataset...\n")
            train_seqs = np.load(self.path+"%s_train_seq.npy"%file_class, allow_pickle = True)
            valid_seqs = np.load(self.path+"%s_valid_seq.npy"%file_class, allow_pickle = True)
    ...
    def get_random_evaluate_data(self, seq_len = 100, use_train = False, file_class = "prs", gap_len = 20, use_popular = True):
        """
        Process the sequence data for testing random target
        """
        if use_train == True and file_class == "prs":
            item_set = set(np.load(self.path+"prs_targets.npy"))
        else:
...
#            users.append(user_id)
    ...
    def get_random_evaluate_data_c(self, seq_len = 100, use_train = False, file_class = "prs", gap_len = 20, use_popular = True, fv_dict = None, mu = None):
        """
...
class DataLoaderPRS(DataLoader):
    """
    Class for PrsNN dataloader  
    """
    def __init__(self, *args, **kwargs):
        super(DataLoaderPRS, self).__init__(*args, **kwargs)
    ...
    def _random_collate_fn(self, batch):
        ...
        for row in batch:
            seq = np.array(row[0])
            start_pos = get_start_index(seq) # Find the first non-zero, since the sequence is prepadding
            length = len(seq) - start_pos # Find the length of the actual sequence
            ...
            seqs.append(new_seq)
            users.append(row[1])
        return torch.LongTensor(seqs), torch.LongTensor(users)
...
class DataLoaderEvalPrs(DataLoader):
    """
    Class for evaluating PrsNN dataloader 
    
    Shape of eval_data: [new_seq, user_id, random_target, label]
    """
    def __init__(self, seq_len, gap_len, *args, **kwargs):
        super(DataLoaderEvalPrs, self).__init__(*args, **kwargs)
        self.collate_fn = self._collate_fn 
    ...
    def _collate_fn(self, batch):
        ...
        for row in batch:
            ...
            labels.append(row[3])
        return  raw_seqs, torch.LongTensor(seqs), torch.LongTensor(users), torch.LongTensor(targets), torch.tensor(labels), 
    ...
    def __iter__(self):
        ...
        if not self.get_path:
            while not finished:
                ...
                idx_target = np.array([seqs[j][starts[j]] for j in range(self.batch_size)])
                for i in range(minlen - 1):
                    # Build inputs & targets
                    idx_input = idx_target
                    idx_target = np.array([seqs[j][starts[j]+i+1] for j in range(self.batch_size)])
            ...
            while not finished:
                ...
                for i in range(minlen):
                    # Build inputs & targets
                    idx_input = np.array([seqs[j][starts[j]+i] for j in range(self.batch_size)])
                    input = torch.LongTensor(idx_input)
...
#class DataLoaderEvalSession(DataLoader):
#    def __init__(self, *args, **kwargs):
#        super(DataLoaderEvalPrs, self).__init__(*args, **kwargs)
#        self.collate_fn = self._collate_fn     
#    
#    def _collate_fn(self, batch):
#        seqs = [seq[-1] for seq in batch]
#        return torch.LongTensor(seqs)
...
class DataLoaderEvalNN1(DataLoader):
    ...
    def _collate_fn(self, batch):
        ...
        for row in batch:        
            ...
            l_path.append(row[4])
        return torch.LongTensor(histories), torch.LongTensor(new_seqs), \
            torch.tensor(targets),  torch.tensor(start_pos), torch.tensor(l_path)
...
##    eval_data  = dp.get_random_evaluate_data(seq_len = config.max_len, use_train = True, file_class = "prs", gap_len = 0)
    
    
#    result_save_dir = "./results/" + config.dataset + '/' + 'prsnn' + '/' 
#    histories = np.load(result_save_dir+"histories.npy", allow_pickle = True)
#    targets = np.load(result_save_dir+"targets.npy", allow_pickle = True)
#    paths = np.load(result_save_dir+"paths.npy", allow_pickle = True)
#    eval_dataset = DatasetEvalNN1(histories, paths, targets, seq_len = 100)
    
#    train_prs = dp.get_prs_nn1_data(train) # Get the training and validation data for prs nn1
#    targets, labels, _ = dp.get_prs_random_data(valid) # Get the testing data with random target
#    targets, histories, paths, _ = dp.get_prs_given_data(valid) # Get the testing data with given target
#    
#    
#    # Show overlapping
#    a = set(train.reshape(-1))
#    b = set(valid.reshape(-1))
#    print("Number of items only in testing set: %i"%(len(b)-len(a.intersection(b))))
